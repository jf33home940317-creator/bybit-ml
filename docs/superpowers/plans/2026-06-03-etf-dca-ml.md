...
5. **Task 14 live forced-buy fired every day.** `is_last_trading_day_of_month(sym_df, today_idx)` from `data.cleaner` is correct in backtest (historical df spans many months → `same_month.max()` is the actual last session of that month) but degenerates to always-True in live mode (sym_df is truncated at today, so the latest bar IS the latest bar of its month). On the first real live run all three symbols received FORCED_BUY signals on a mid-month Wednesday with prob ≈ 0.1–0.3 — wiping the entire month's budget. Fix: introduce a calendar-based helper `_is_calendar_month_last_business_day(today)` in `live/predict_daemon.py` (`today` is Mon–Fri AND next business day is in next calendar month) and use it in the live daemon. Backtest engine still uses the original sym_df-based check (correct for full historical data). Missing edge case: month-end Thursday when Friday is a market holiday — those months will simply not see a forced buy, an acceptable cost since 1st-of-month retrain resets budget anyway.
