...
def test_compute_indicators_produces_expected_columns():
    ...
    close = pd.Series(100 + np.cumsum(rng.normal(0, 1, n))).abs() + 50
    df = pd.DataFrame({
        "open": close, "high": close + 1, "low": close - 1,
        "close": close, "volume": rng.integers(1e6, 1e7, n),
    }, index=pd.bdate_range("2023-01-01", periods=n, name="date"))
...
def test_builder_combines_tech_macro_labels():
    ...
    close = pd.Series(100 + np.linspace(0, 10, 400), index=idx)
